--[[
    ...
    Created by MadStudio (loleris)
    
    Do not modify this script unless you know exactly what you are doing.
    Usage: require(path.to.ProfileService)
]]

local ProfileService = {
	ServiceLocked = false, -- Set to true once the server is shutting down
	IssueSignal = nil,
	CorruptionSignal = nil,
	CriticalStateSignal = nil,
	CriticalState = false,
	ServiceIssueCount = 0,
...
do
	...
	Madwork = {
		...
		ConnectToOnClose = function(task_fn, run_in_studio_mode)
			if game:GetService("RunService"):IsStudio() == false or run_in_studio_mode == true then
				game:BindToClose(task_fn)
			end
		end,
	}
end

ProfileService.IssueSignal = Madwork.NewScriptSignal()
ProfileService.CorruptionSignal = Madwork.NewScriptSignal()
ProfileService.CriticalStateSignal = Madwork.NewScriptSignal()
...
local function WaitForLiveAccessCheck()
	while IsLiveCheckActive == true do task.wait() end
end

local function WaitForPendingProfileStore(profile_store)
	while profile_store._is_pending == true do task.wait() end
end
...
local function NewMockDataStoreKeyInfo(params)
	...
	return {
		...
		GetMetadata = function() return DeepCopyTable(meta_data) end,
		GetUserIds = function() return DeepCopyTable(user_ids) end,
	}
end
...
local function MockUpdateAsync(mock_data_store, profile_store_name, key, transform_function, is_get_call)
	...
	if mock_entry == nil then
		...
		if is_get_call ~= true then
			mock_entry = {Data = nil, CreatedTime = epoch_time, UpdatedTime = epoch_time, VersionId = 0, UserIds = {}, MetaData = {}}
...
local function StandardProfileUpdateAsyncDataStore(profile_store, profile_key, update_settings, is_user_mock, is_get_call, version)
	...
	local success, error_message = pcall(function()
		local transform_function = function(latest_data)
			...
			if data_corrupted == true then latest_data.WasCorrupted = true end
				...
				if is_get_call == true then
					...
					if version ~= nil then
						...
						if success == false and type(error_message) == "string" and string.find(error_message, "not valid") ~= nil then
							warn("[ProfileService]: Passed version argument is not valid")
						end
					else
	...
	if success == true and type(loaded_data) == "table" then
		...
		RegisterIssue(error_message or "Undefined error", profile_store._profile_store_name, profile_store._profile_store_scope, profile_key)
		return nil
	end
end
...
local function RemoveProfileFromAutoSave(profile)
	...
	if auto_save_index ~= nil then
		...
		if AutoSaveList[AutoSaveIndex] == nil then AutoSaveIndex = 1 end
	end
end
		...
		local loaded_data, key_info = StandardProfileUpdateAsyncDataStore(profile._profile_store, profile._profile_key, {
			...
			EditProfile = function(latest_data)
				...
				if session_owns_profile == true then
					if is_overwriting ~= true then
						local latest_global_updates_list = latest_data.GlobalUpdates[2]
						local global_updates_object = profile.GlobalUpdates
						local pending_update_lock = global_updates_object._pending_update_lock
						local pending_update_clear = global_updates_object._pending_update_clear
...
function GlobalUpdates:ListenToNewActiveUpdate(listener)
	if type(listener) ~= "function" then error("[ProfileService]: Listener must be a function") end
	local profile = self._profile
	if self._update_handler_mode == true then error("[ProfileService]: Cannot listen in GlobalUpdateProfileAsync") end
	if self._new_active_update_listeners == nil then error("[ProfileService]: Cannot listen in view mode") end
	if profile:IsActive() == false then return {Disconnect = function() end} end
...
function GlobalUpdates:ListenToNewLockedUpdate(listener)
	...
	local profile = self._profile
	if self._update_handler_mode == true then error("[ProfileService]: Cannot listen in GlobalUpdateProfileAsync") end
	if self._new_locked_update_listeners == nil then error("[ProfileService]: Cannot listen in view mode") end
	if profile:IsActive() == false then return {Disconnect = function() end} end
...
function GlobalUpdates:LockActiveUpdate(update_id)
	...
	local profile = self._profile
	if self._update_handler_mode == true then error("[ProfileService]: Cannot lock in GlobalUpdateProfileAsync") end
	if self._pending_update_lock == nil then error("[ProfileService]: Cannot lock in view mode") end
	if profile:IsActive() == false then error("[ProfileService]: Profile expired") end
...
function GlobalUpdates:ClearLockedUpdate(update_id)
	if type(update_id) ~= "number" then error("[ProfileService]: Invalid update_id") end
	local profile = self._profile
	if self._update_handler_mode == true then error("[ProfileService]: Cannot clear in GlobalUpdateProfileAsync") end
	if self._pending_update_clear == nil then error("[ProfileService]: Cannot clear in view mode") end
	if profile:IsActive() == false then error("[ProfileService]: Profile expired") end
	local global_update_exists = nil
	for _, global_update in ipairs(self._updates_latest[2]) do
		if global_update[1] == update_id then
...
function GlobalUpdates:AddActiveUpdate(update_data)
	...
	if self._new_active_update_listeners ~= nil then error("[ProfileService]: Use GlobalUpdateProfileAsync to add updates") end
	if self._update_handler_mode ~= true then error("[ProfileService]: Use GlobalUpdateProfileAsync to add updates") end
...
function GlobalUpdates:ChangeActiveUpdate(update_id, update_data)
	if type(update_id) ~= "number" then error("[ProfileService]: Invalid update_id") end
	if type(update_data) ~= "table" then error("[ProfileService]: Invalid update_data") end
	if self._new_active_update_listeners ~= nil then error("[ProfileService]: Use GlobalUpdateProfileAsync to change updates") end
	if self._update_handler_mode ~= true then error("[ProfileService]: Use GlobalUpdateProfileAsync to change updates") end
	local updates_latest = self._updates_latest
	...
	if get_global_update ~= nil then
		if get_global_update[3] == true then error("[ProfileService]: Cannot change locked global update") end
...
function GlobalUpdates:ClearActiveUpdate(update_id)
	if type(update_id) ~= "number" then error("[ProfileService]: Invalid update_id") end
	if self._new_active_update_listeners ~= nil then error("[ProfileService]: Use GlobalUpdateProfileAsync to clear updates") end
	if self._update_handler_mode ~= true then error("[ProfileService]: Use GlobalUpdateProfileAsync to clear updates") end
	local updates_latest = self._updates_latest
	...
	if get_global_update ~= nil then
		if get_global_update[3] == true then error("[ProfileService]: Cannot clear locked global update") end
...
function Profile:GetMetaTag(tag_name)
	...
	if meta_data == nil then return nil end
	return self.MetaData.MetaTags[tag_name]
end

function Profile:SetMetaTag(tag_name, value)
	if type(tag_name) ~= "string" or string.len(tag_name) == 0 then error("[ProfileService]: Invalid tag_name") end
...
function Profile:Save()
	if self._view_mode == true then error("[ProfileService]: Cannot save in view mode") end
	if self:IsActive() == false then
		warn("[ProfileService]: Attempted saving an inactive profile " .. self:Identify())
		return
	end
...
function Profile:AddUserId(user_id)
	if type(user_id) ~= "number" or user_id % 1 ~= 0 then
		warn("[ProfileService]: Invalid UserId")
		return
	end
	if user_id < 0 and self._is_user_mock ~= true and UseMockDataStore ~= true then return end
...
function Profile:ClearGlobalUpdates()
	if self._view_mode ~= true then error("[ProfileService]: Can only be used in view mode") end
...
function ProfileVersionQuery:NextAsync(_is_stacking)
	...
	if self._is_query_yielded == false then
		...
		if _is_stacking == true then table.insert(self._query_queue, 1, query_job) else table.insert(self._query_queue, query_job) end
	end
	while is_finished == false do task.wait() end
	return profile
end
...
function ProfileStore:LoadProfileAsync(profile_key, not_released_handler, _use_mock)
	...
	if self._profile_template == nil then error("[ProfileService]: Profile template not set") end
	if type(profile_key) ~= "string" or string.len(profile_key) == 0 then error("[ProfileService]: Invalid profile_key") end
	if type(not_released_handler) ~= "function" and not_released_handler ~= "ForceLoad" and not_released_handler ~= "Steal" then error("[ProfileService]: Invalid not_released_handler") end
	...
	for _, profile_store in ipairs(ActiveProfileStores) do
		if profile_store._profile_store_lookup == self._profile_store_lookup then
			...
			if loaded_profiles[profile_key] ~= nil then error("[ProfileService]: Profile already loaded in this session") end
		end
	end
	...
	while ProfileService.ServiceLocked == false do
		...
		if profile_load_job ~= nil then
			...
			while profile_load_job[2] == nil do task.wait() end
			...
			profile_load_job[2] = table.pack(StandardProfileUpdateAsyncDataStore(self, profile_key, {
					...
					if ProfileService.ServiceLocked == false then
						...
						elseif type(active_session) == "table" then
							if IsThisSession(active_session) == false then
								...
								if last_update ~= nil and os.time() - last_update > SETTINGS.AssumeDeadSessionLock then
									latest_data.MetaData.ActiveSession = {PlaceId, JobId}
									latest_data.MetaData.ForceLoadSession = nil
									return
								end
				...
				MissingProfileHandle = function(latest_data)
					...
					latest_data.MetaData = {ProfileCreateTime = os.time(), SessionLoadCount = 0, ActiveSession = {PlaceId, JobId}, ForceLoadSession = nil, MetaTags = {}}
				end,
				EditProfile = function(latest_data)
					if ProfileService.ServiceLocked == false then
						local active_session = latest_data.MetaData.ActiveSession
...
function ProfileStore:ViewProfileAsync(profile_key, version, _use_mock)
	...
	if ProfileService.ServiceLocked == true then return nil end
	WaitForPendingProfileStore(self)
	if version ~= nil and (_use_mock == UseMockTag or UseMockDataStore == true) then return nil end
	while ProfileService.ServiceLocked == false do
		local loaded_data, key_info = StandardProfileUpdateAsyncDataStore(self, profile_key, {
			ExistingProfileHandle = nil,
			MissingProfileHandle = function(latest_data)
				latest_data.Data = DeepCopyTable(self._profile_template)
				latest_data.MetaData = {ProfileCreateTime = os.time(), SessionLoadCount = 0, ActiveSession = nil, ForceLoadSession = nil, MetaTags = {}}
			end,
			EditProfile = nil,
		}, _use_mock == UseMockTag, true, version)
		CustomWriteQueueMarkForCleanup(self._profile_store_lookup, profile_key)
		if loaded_data ~= nil then
			if key_info == nil then return nil end
			local global_updates_object = {
				_updates_latest = loaded_data.GlobalUpdates,
				_profile = nil,
			}
...
function ProfileStore:ProfileVersionQuery(profile_key, sort_direction, min_date, max_date, _use_mock)
	if type(profile_key) ~= "string" or string.len(profile_key) == 0 then error("[ProfileService]: Invalid profile_key") end
	if ProfileService.ServiceLocked == true then return setmetatable({}, ProfileVersionQuery) end
	WaitForPendingProfileStore(self)
	if _use_mock == UseMockTag or UseMockDataStore == true then error("[ProfileService]: Version query not supported in mock mode") end
	if sort_direction ~= nil and (typeof(sort_direction) ~= "EnumItem" or sort_direction.EnumType ~= Enum.SortDirection) then error("[ProfileService]: Invalid sort_direction") end
...
function ProfileService.GetProfileStore(profile_store_index, profile_template)
	...
	elseif type(profile_store_index) == "table" then
		...
		error("[ProfileService]: Invalid profile_store_index")
	end
	if profile_store_name == nil or type(profile_store_name) ~= "string" or string.len(profile_store_name) == 0 then error("[ProfileService]: Invalid Name") end
	if profile_store_scope ~= nil and (type(profile_store_scope) ~= "string" or string.len(profile_store_scope) == 0) then error("[ProfileService]: Invalid Scope") end
	if type(profile_template) ~= "table" then error("[ProfileService]: Invalid profile_template") end
	local profile_store = {
		Mock = {
			LoadProfileAsync = function(_, profile_key, not_released_handler) return profile_store:LoadProfileAsync(profile_key, not_released_handler, UseMockTag) end,
			GlobalUpdateProfileAsync = function(_, profile_key, update_handler) return profile_store:GlobalUpdateProfileAsync(profile_key, update_handler, UseMockTag) end,
			ViewProfileAsync = function(_, profile_key, version) return profile_store:ViewProfileAsync(profile_key, version, UseMockTag) end,
			FindProfileVersionAsync = function(_, profile_key, sort_direction, min_date, max_date) return profile_store:FindProfileVersionAsync(profile_key, sort_direction, min_date, max_date, UseMockTag) end,
			WipeProfileAsync = function(_, profile_key) return profile_store:WipeProfileAsync(profile_key, UseMockTag) end
		},
...
if IsStudio == true then
	...
	task.spawn(function()
		...
		if no_internet_access == true then warn("[ProfileService]: No internet access") end
...
RunService.Heartbeat:Connect(function()
		...
		while os_clock - LastAutoSave > auto_save_index_speed do
			...
			if os_clock - profile._load_timestamp < SETTINGS.AutoSaveProfiles then
				...
				for _ = 1, auto_save_list_length - 1 do
					...
					if os_clock - profile._load_timestamp >= SETTINGS.AutoSaveProfiles then break else profile = nil end
				end
			end
			AutoSaveIndex = AutoSaveIndex + 1
			if AutoSaveIndex > auto_save_list_length then AutoSaveIndex = 1 end
			if profile ~= nil then task.spawn(SaveProfileAsync, profile) end
		end
	end
	...
	while true do
		...
		if issue_time == nil then break
		elseif os.clock() - issue_time > SETTINGS.IssueLast then table.remove(IssueQueue, 1)
		else break end
	end
end)
...
task.spawn(function()
	...
	Madwork.ConnectToOnClose(function()
		...
		for index, profile in ipairs(AutoSaveList) do active_profiles[index] = profile end
		...
		while on_close_save_job_count > 0 or ActiveProfileLoadJobs > 0 or ActiveProfileSaveJobs > 0 do task.wait() end
		return
